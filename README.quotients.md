...
Note: we only support proper proof generation for unix-based systems in our extension to PUMPKIN Pi, since timeouts are only supported in those instances.
...
We tested our case studies on the cubical library version v0.7, both with Agda version v2.6.4.1 on Ubuntu 24.04.2 and v2.6.4.3 on Ubuntu 22.04.5.
...
- [`plugin/case-studies/equivalence_int_abs.agda`](plugin/case-studies/equivalence_int_abs.agda) (for the internal proofs of correct repair in Cubical Agda). [`plugin/case-studies/alternateFunExtDep.agda`](plugin/case-studies/alternateFunExtDep.agda) must be compiled prior.

## Using the Plugin

To repair across a setoid equivalence using this extension to PUMPKIN Pi, first the user must define a configuration. This involves four components for each type:
...
- iota-reduction rules, which are theorems stating how an application of an eliminator to a constructor reduces
...
This tells PUMPKIN Pi that `depConstrA1` should be repaired to `depConstrB1`, and similarly for the other lists. There is no limit on the length of the lists of constrs, elims, or iotas, but the lengths of the lists for A for each element of the configuration need to be the same as the length of the corresponding list for B. It is possible to reconfigure PUMPKIN Pi to use a different set of eliminators. This is especially useful when eliminators need to have their motives specialized to provide additional data, such as a proof that motive is proper. We do this multiple times in our case studies.
