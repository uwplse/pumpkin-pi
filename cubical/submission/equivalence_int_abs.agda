...
NatIsoInt/rInt : Iso Nat (Int / rInt)
...
depElimInt/rInt P set baseCase sucCase = elimProp set lem where
  ...
  lem (neg zero) = subst P (eq/ (pos zero) (neg zero) refl) baseCase
...
elimOK a b a≡b PA PB PBSet PA≡PB PAO PBO PAO≡PBO PAS PBS PAS≡PBS =
  J -- adjust a≡b from pathP to path to make it easy to use JDep
    ...
    (JDep -- adjust to a homogeneous PathP about proofs about a
       ...
       (cong (depElimSetInt/rInt PB PBSet PBO PBS) (fromPathP a≡b)))
...
-- open import Cubical.Foundations.Isomorphism
-- transportIsoToPath : {A B : Type} (f : Iso A B) (x : A) → transport (isoToPath f) x ≡ Iso.fun f x
...
--     VecVeczSection v = {!!} where
--       f'' : (a : Int / rInt) (v : Vecz T a) → Vecz T (transport (λ i → Nat≡Int/rInt i) (transport⁻ (λ i → Nat≡Int/rInt i) a))
--       f'' a v = {!!}
--       typeIso : (a : Int / rInt) → Iso (Vecz T a) (Vecz T (transport (λ i → Nat≡Int/rInt i) (transport⁻ (λ i → Nat≡Int/rInt i) a)))
--       typeIso a = subst (λ x → Iso (Vecz T a) (Vecz T x)) (sym (transportTransport⁻ Nat≡Int/rInt a)) idIso
--       typeEquiv : (a : Int / rInt) → Vecz T a ≡ Vecz T (transport (λ i → Nat≡Int/rInt i) (transport⁻ (λ i → Nat≡Int/rInt i) a))
--       typeEquiv a = isoToPath (typeIso a)
--       help : (a : Int / rInt) (v : Vecz T a) → VecToVecz (VeczToVec (transport (λ i → typeEquiv a i) v)) ≡ transport (λ i → typeEquiv a i) v
--       help a v =
--         elimVecz
--           (λ (a : Int / rInt) (v : Vecz T a) → VecToVecz (VeczToVec (transport (λ i → typeEquiv a i) v)) ≡ transport (λ i → typeEquiv a i) v)
--           (subst (λ x → VecToVecz (VeczToVec x) ≡ x) (sym (transportIsoToPath (typeIso depConstrInt/rInt0) nilz)) {!Iso.fun (typeIso depConstrInt/rInt0)!})
--           (λ n t v2 IH → {!!})
--           {!!}
--           {!!}  -- TODO: Finish proof of section.
--     ---
