 ...
 * add0LZ. Then, when we go to repair this term, we can supply the 
 ...
 * so we need to ensure that the terms we repair only have one kind of eliminator in them.
 * We will see how work around this to repair a term with multiple kinds of eliminators later.
 *)
...
(* 
 * Now, we will repair add0LZ. This theorem uses depElimPropGZ in its proof. 
 * As such, we cannot immediately repair it. Instead, we need to specialize
 * depElimPropGZ to use the repaired motive for the specialized depElimPropZ
 ...
 * that it produces the same output as the repaired addition function.
