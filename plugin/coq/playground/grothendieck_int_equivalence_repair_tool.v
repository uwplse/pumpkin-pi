...
Definition Z := IndInt.Z.

(* 
 * We define the side of the configuration corresponding to this type.
 *)

Definition depConstrIndIntPos (n : nat) : Z := IndInt.pos n.
Definition depConstrIndIntNegSuc (n : nat) : Z := IndInt.negsuc n.
...
Definition f (z : GZ) : Z :=
  depRecGZ Z (fun n => IndInt.pos n) (fun n => IndInt.negsuc n) z.
...
Proof.
  ...
  - unfold f.
    simpl.
    rewrite <- (iotaRecGZPosEq Z (fun n => IndInt.pos n) (fun n => IndInt.negsuc n)).
    reflexivity.
  - unfold f.
    simpl.
    rewrite <- (iotaRecGZNegSucEq Z (fun n => IndInt.pos n) (fun n => IndInt.negsuc n)).
    reflexivity.
Qed.
