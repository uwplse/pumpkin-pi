...
for f in $(find out/normalized/*.out); do
  ...
  sed -i "s/(\* TIME-SEARCH $name \*)/Redirect \"..\/out\/$dirname\/${suffix}1\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree1 Elem.x in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}10\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree10 Elem.x in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}100\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree100 Elem.x in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}1000\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree1000 Elem.x in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}10000\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree10000 Elem.x in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}100000\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree100000 Elem.x in tt)./" equiv4free/main2.v
  
  sed -i "s/(\* TIME-AVL $name \*)/Redirect \"..\/out\/$dirname\/${suffix}1\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree1 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}10\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree10 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}100\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree100 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}1000\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree1000 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}10000\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree10000 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}100000\" Time Eval vm_compute in (let foo := $defname _ _ _ _ tree100000 in tt)./" equiv4free/main2.v
  
  sed -i "s/(\* TIME-SIZED $name \*)/Redirect \"..\/out\/$dirname\/${suffix}1\" Time Eval vm_compute in (let foo := $defname tree1 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}10\" Time Eval vm_compute in (let foo := $defname tree10 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}100\" Time Eval vm_compute in (let foo := $defname tree100 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}1000\" Time Eval vm_compute in (let foo := $defname tree1000 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}10000\" Time Eval vm_compute in (let foo := $defname tree10000 in tt).\n\tRedirect \"..\/out\/$dirname\/${suffix}100000\" Time Eval vm_compute in (let foo := $defname tree100000 in tt)./" equiv4free/main2.v
done
