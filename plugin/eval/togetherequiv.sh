...
for f in $(find out/normalized/*.out); do
  ...
  if [ $defname == "search'" ]
  then 
     sed -i "s/(\* TIME-SMALL $name \*)/Redirect \"..\/out\/$dirname\/${suffix}20\" Time Eval vm_compute in ($defname _ _ _ _ tree20 Elem.x).\n\tRedirect \"..\/out\/$dirname\/${suffix}40\" Time Eval vm_compute in ($defname _ _ _ _ tree40 Elem.x).\n\tRedirect \"..\/out\/$dirname\/${suffix}60\" Time Eval vm_compute in ($defname _ _ _ _ tree60 Elem.x).\n\tRedirect \"..\/out\/$dirname\/${suffix}80\" Time Eval vm_compute in ($defname _ _ _ _ tree80 Elem.x).\n\tRedirect \"..\/out\/$dirname\/${suffix}100\" Time Eval vm_compute in ($defname _ _ _ _ tree100 Elem.x)./" equiv4free/mainequiv2.v
  else
    sed -i "s/(\* TIME-SMALL $name \*)/Redirect \"..\/out\/$dirname\/${suffix}20\" Time Eval vm_compute in ($defname _ _ _ _ tree20).\n\tRedirect \"..\/out\/$dirname\/${suffix}40\" Time Eval vm_compute in ($defname _ _ _ _ tree40).\n\tRedirect \"..\/out\/$dirname\/${suffix}60\" Time Eval vm_compute in ($defname _ _ _ _ tree60).\n\tRedirect \"..\/out\/$dirname\/${suffix}80\" Time Eval vm_compute in ($defname _ _ _ _ tree80).\n\tRedirect \"..\/out\/$dirname\/${suffix}100\" Time Eval vm_compute in ($defname _ _ _ _ tree100)./" equiv4free/mainequiv2.v
