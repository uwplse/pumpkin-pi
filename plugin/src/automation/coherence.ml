...
let prove_coherence env sigma orn =
  ...
  let a = mkRel 1 in
  let is = on_red_type_default (fun _ sigma trm -> unfold_args (EConstr.to_constr sigma trm)) env_coh sigma (EConstr.of_constr a) in
  let b_sig = mkAppl (orn.promote, snoc a is) in
  let b_sig_typ = on_red_type_default (fun _ sigma trm -> dest_sigT (EConstr.to_constr sigma trm)) env_coh sigma (EConstr.of_constr b_sig) in
