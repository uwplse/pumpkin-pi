...
let rec eta_extern env evm idset term =
  ...
  | Case (info, motive, discr, cases) ->
    ...
    let (((rec_decl :: idx_ctxt) as ret_ctxt), ret_idset), ret_type =
      Term.decompose_lam_n_assum (nidx + 1) motive |> on_fst (freshen_context idset)
    in
    let branches =
      Array.map2 Term.decompose_lam_n_assum info.ci_cstr_nargs cases |>
  ...
  | Fix (([|rec_pos|], 0), ([|Name fix_id|], [|fun_type|], [|fun_body|])) ->
    let (((rec_decl :: _) as dom_ctxt), idset), cod_type =
      Term.decompose_prod_n_assum (rec_pos + 1) fun_type |> on_fst (freshen_context idset)
    in
    ...
    let fix_body = Term.decompose_lam_n_assum (rec_pos + 1) fun_body |> snd in
  ...
  | CoFix (0, ([|Name fix_id|], [|fun_type|], [|fun_body|])) ->
    let (dom_ctxt, idset), cod_type =
      Term.decompose_prod_assum fun_type |> on_fst (freshen_context idset)
    in
    let fix_decl = rel_assum (Name fix_id, recompose_prod_assum dom_ctxt cod_type) in
    let fix_ctxt = context_app dom_ctxt [fix_decl] in
    let fix_body = Term.decompose_lam_n_assum (Rel.length dom_ctxt) fun_body |> snd in
    let fix_idset = Id.Set.add fix_id idset in
    CCoFix
      (CAst.make fix_id,
       [(CAst.make fix_id,
...
and eta_extern_pattern idset npar head ctxt =
  let open Context in
  let open Constrexpr in
